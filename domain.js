...
export const claimEvidence=[['F06','F07','F14'],['F09'],['F06','F09','F14']];
...
export function checkBackup(d){return ['candidate','r1','r2','r3'].filter(k=>d[k]!=={candidate:'R2',r1:'duplicate_order',r2:'valid',r3:'overwritten_original'}[k]);}
export function checkTimeline(d,claims){const errors=[];if(d.length!==5||d.some((c,i)=>c.id!==i))errors.push({id:'order',message:'사건 순서를 다시 확인하세요.'});claims.forEach((c,i)=>{const wrong=[];if(c.judgment!==['contradicted','contradicted','undetermined'][i])wrong.push('판정');const hasEvidence=i===2?claimEvidence[i].some(r=>c.refs.includes(r)):claimEvidence[i].every(r=>c.refs.includes(r));if(!hasEvidence)wrong.push('판정을 뒷받침하는 근거');if(i===2&&c.reason!=='account_not_person')wrong.push('이유');if(wrong.length)errors.push({id:i,message:'다시 확인할 항목: '+wrong.join(' · ')});});return errors;}
...
export function validate(s,boundary=null){
 ...
 const b=s.backupDraft;
 // Older v3 saves have no R2 assessment. Preserve solved progress and leave unfinished drafts unanswered.
 if(b&&typeof b==='object'&&!Object.hasOwn(b,'r2'))b.r2=s.stage>=4?'valid':'';
 if(!b||!['','R1','R2','R3'].includes(b.candidate)||['r1','r2','r3'].some(k=>!['',...Object.keys(backupReasons)].includes(b[k])))bad('백업 초안이 손상되었습니다.');
