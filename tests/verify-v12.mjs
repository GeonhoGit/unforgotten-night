...
assert.equal(checkBackup({candidate:'R2',r1:'duplicate_order',r2:'valid',r3:'overwritten_original'}).length,0);
assert.deepEqual(checkBackup({candidate:'R2',r1:'duplicate_order',r2:'valid',r3:'copy_error'}),['r3']);
const s=fresh();s.started=true;s.stage=3;assert.throws(()=>prepareHandoff(s));s.backupDraft={candidate:'R2',r1:'duplicate_order',r2:'valid',r3:'overwritten_original'};s.stage=4;s.cpWitness=snapshot(s);s.opened=['F13'];s.policy='protect';
